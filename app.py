...
if auth_mode == "新規登録":
        ...
        if submit_button:
            if new_username and new_display_name and new_password:
                ...
                try:
                    ...
                    st.error("登録エラー: IDが既に使用されている可能性があります")
            else:
    ...
    if auth_status:
        ...
        # 履歴データの取得
        log_res = conn.table("logs").select("*").eq("username", username).order("date", desc=True).execute()
        ...
        with st.sidebar.expander("全体の目標設定"):
            new_target_name = st.text_input("目標の名前", EX_NAME)
            new_target_date = st.date_input("最終目標日", EX_DATE)
            new_target_goal = st.number_input("目標総時間(h)", value=int(GOAL_H), min_value=1)
            if st.button("全体目標を更新"):
                conn.table("config").upsert({
                    "username": username, "exam_name": new_target_name,
                    "exam_date": str(new_target_date), "goal_hours": new_target_goal
                }).execute()
                st.rerun()
        ...
        with st.sidebar.expander("検定・イベント管理"):
            ...
            if st.button("イベント登録"):
                if ev_name:
                    conn.table("events").insert({
                        "username": username, "event_name": ev_name, "event_date": str(ev_date)
                    }).execute()
                    st.rerun()
            ...
            if not df_events.empty:
                for _, ev in df_events.iterrows():
                    ...
                    if ec2.button("削", key=f"ev_del_{ev['id']}"):
        ...
        st.divider()
        tab1, tab2, tab3 = st.tabs(["学習推移", "履歴編集・削除", "教科分析"])
        ...
        with tab3:
            if not df_log.empty:
                ...
                if not sub_df.empty:
                    ...
                    st.write(f"累計学習時間: {sub_df['hours'].sum():.1f} 時間")
